...
if (rex_post('formsubmit', 'string') == '1') {
    ...
    if ($this->getConfig('domainprofiles_enabled')) {
        ...
        foreach ($domains as $domain) {
            $domainprofiles[] =  ['agency--'.$domain->getValue('id'), 'string'];
            $domainprofiles[] =  ['file--'.$domain->getValue('id'), 'string'];
            $domainprofiles[] =  ['file2--'.$domain->getValue('id'), 'string'];
            $domainprofiles[] =  ['textarea--'.$domain->getValue('id'), 'string'];
            $domainprofiles[] =  ['border_text--'.$domain->getValue('id'), 'string'];
            $domainprofiles[] =  ['border_color--'.$domain->getValue('id'), 'string'];
            $domainprofiles[] =  ['color1--'.$domain->getValue('id'), 'string'];
            $domainprofiles[] =  ['color2--'.$domain->getValue('id'), 'string'];
...
// Wenn Domainprofile aktiviert sind, dann starten wir hier einfach nochmal frisch
if ($this->getConfig('domainprofiles_enabled')) {
    $content = '';
    ...
    foreach ($domains as $domain) {
        $domain_i++;
        $active = '';
        $domain_id = $domain->getValue('id');
        if ($domain_i == 1) {
            $active = ' active';
        }
        $content .= '<li role="presentation" class="' . $active . '">
                                    <a href="#be_branding-branding-id--' . $domain_id . '" aria-controls="#be_branding-branding-id--' . $domain_id . '" role="tab" data-toggle="tab">
                                        ' . $domain->getValue('domain') . '
                                    </a>
                                </li>';
    } // Ende foreach
    $content .= '    </ul>
    </div>';// Ende Tab-Navigation
    
// Beginn Config-Inhalt pro Domain
    ...
    foreach ($domains as $domain) {
        ...
        #dump($domain);
        $content .= '<div role="tabpanel" class="tab-pane' . $active . '" id="be_branding-branding-id--' . $domain_id . '">';

        $content .= '<h3>'.$domain->getValue('domain').'</h3>';
        $content .= '<fieldset><legend>Farbschema</legend>';
...
// Einfaches Textfeld
        ...
        $n = [];
        $n['label'] = '<label for="be_branding-config-color1">Prim&auml;rfarbe<p><small>z.B. HG-Farbe des Redaxo-Headers</small></p></label>';
        $n['field'] = '<input class="form-control'.$class_colorpicker.'" type="text" id="be_branding-config-color1" name="config[color1--'.$domain_id.']" value="' . $this->getConfig('color1--'.$domain_id) . '" placeholder="z.B. rgba(255, 100, 0, 1)"/><p class="help-block rex-note">Beliebige RGBa-Farbangabe (z.B. <code>rgba(255, 100, 0, 0.5)</code>)</p>';
        $formElements[] = $n;
...
// Einfaches Textfeld
        ...
        $n = [];
        $n['label'] = '<label for="be_branding-config-color2">Sekund&auml;rfarbe<p><small>Farbe f&uuml;r das Redaxo-Logo</small></p></label>';
        $n['field'] = '<input class="form-control'.$class_colorpicker.'" type="text" id="be_branding-config-color2" name="config[color2--'.$domain_id.']" value="' . $this->getConfig('color2--'.$domain_id) . '" placeholder="z.B. rgba(255, 100, 0, 1)"/><p class="help-block rex-note">Beliebige RGBa-Farbangabe (z.B. <code>rgba(255, 100, 0, 0.5)</code>)</p>';
        $formElements[] = $n;
...
<div class="rex-js-widget rex-js-widget-media">
	<div class="input-group">
		<input class="form-control" type="text" name="config[file--'.$domain_id.']" value="' . $this->getConfig('file--'.$domain_id) . '" id="REX_MEDIA_'.$domain_id.'1" readonly="readonly">
		<span class="input-group-btn">
        <a href="#" class="btn btn-popup" onclick="openREXMedia('.$domain_id.'1);return false;" title="ÖFFNEN">
        	<i class="rex-icon rex-icon-open-mediapool"></i>
        </a>
        <a href="#" class="btn btn-popup" onclick="addREXMedia('.$domain_id.'1);return false;" title="NEU">
        	<i class="rex-icon rex-icon-add-media"></i>
        </a>
        <a href="#" class="btn btn-popup" onclick="deleteREXMedia('.$domain_id.'1);return false;" title="REMOVE">
        	<i class="rex-icon rex-icon-delete-media"></i>
        </a>
        <a href="#" class="btn btn-popup" onclick="viewREXMedia('.$domain_id.'1);return false;" title="ANSEHEN">
        	<i class="rex-icon rex-icon-view-media"></i>
        </a>
        </span>
	</div>
...
// Wenn REX 5.12 - erst ab hier gibt es den Login-Screen mit BG
        if(rex_string::versionCompare(rex::getVersion(), '5.12', '>=')) {
            ...
            $elements = array();
            $elements['label'] = '
	         <label for="be-branding-login-bg-option">Hintergrund Login-Screen</label>';

            // create select
            $select = new rex_select;
            $select->setId('be-branding-login-bg-option--'.$domain_id);
            $select->setSize(1);
            $select->setAttribute('class', 'form-control');
            $select->setName('config[login_bg_setting--'.$domain_id.']');
            // add options
            $select->addOption('Eigenes Hintergrundbild', 'own_bg');
            $select->addOption('Primärfarbe aus Farbschema', 'primary_bg');
            $select->addOption('Sekundärfarbe aus Farbschema', 'secondary_bg');
            $select->addOption('Farbverlauf aus Primär-und Sekundärfarbe aus Farbschema', 'gradient_bg');
            $select->addOption('REDAXO-Standard-Hintergrundbild', 'redaxo_standard_bg');
            $select->setSelected($this->getConfig('login_bg_setting--'.$domain_id));
            $elements['field'] = $select->get();
            $formElements[] = $elements;
            // parse select element
            $fragment = new rex_fragment();
            ...
            $content .= '<div id="be-branding-login-bg-setting--'.$domain_id.'">';
            $formElements = [];
            $n = [];
            $n['label'] = '<label for="REX_MEDIA_3">Hintergrundbild des Login-Screens</label>';
	...
	<div class="rex-js-widget rex-js-widget-media">
		<div class="input-group">
			<input class="form-control" type="text" name="config[login_bg--'.$domain_id.']" value="' . $this->getConfig('login_bg--'.$domain_id) . '" id="REX_MEDIA_'.$domain_id.'3" readonly="readonly">
			<span class="input-group-btn">
			<a href="#" class="btn btn-popup" onclick="openREXMedia('.$domain_id.'3);return false;" title="ÖFFNEN">
				<i class="rex-icon rex-icon-open-mediapool"></i>
			</a>
			<a href="#" class="btn btn-popup" onclick="addREXMedia('.$domain_id.'3);return false;" title="NEU">
				<i class="rex-icon rex-icon-add-media"></i>
			</a>
			<a href="#" class="btn btn-popup" onclick="deleteREXMedia('.$domain_id.'3);return false;" title="REMOVE">
				<i class="rex-icon rex-icon-delete-media"></i>
			</a>
			<a href="#" class="btn btn-popup" onclick="viewREXMedia('.$domain_id.'3);return false;" title="ANSEHEN">
				<i class="rex-icon rex-icon-view-media"></i>
			</a>
			</span>
		</div>
...
$(document).on('rex:ready', function() {
    ...
    if ($this->getConfig('domainprofiles_enabled')) {
        foreach($domains as $domain) {
            $domain_id = $domain->getValue('id');
            echo <<<JS
                if($("#be-branding-login-bg-option--$domain_id").val() == 'own_bg') {
                    $('#be-branding-login-bg-setting--$domain_id').show();
                    } else {
                        $('#be-branding-login-bg-setting--$domain_id').hide();
                        }
                    
                $("#be-branding-login-bg-option--$domain_id").change(function() {
                  if(this.value != 'own_bg') {
                      $('#be-branding-login-bg-setting--$domain_id').hide('fast');
                      } else {
                          $('#be-branding-login-bg-setting--$domain_id').show('fast');
                          }
                });
            JS;
